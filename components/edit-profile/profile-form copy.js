...
import { isModelProfileValid } from "../../utils/validations/profile";
...
const ProfileCreationForm = ({ data, userId }) => {
  // const { isOpen, onOpen, onClose } = useDisclosure();
  // const [socialLink, setSocialLink] = useState({});
  // const [socialError, setSocialError] = useState({});
  const [cockStatus, setCockSizeStatus] = useState("");
  const [isPlan, setPlan] = useState("Gent");
  ...
  const handleForSubmit = async () => {
    const dataErrors = isModelProfileValid(values, isPlan);
    console.log(dataErrors, "errororor");
    ...
    setLoading(true);
    try {
      const createResponse = await callPutApi(`/model/${data?.model?._id}`, {
        ...values,
        isAdmin: true,
      });
  ...
  const {
    ...
    initialValues: {
      socialLink: data?.model?.social_media?.socialMediaAccounts || {},
      heading: data?.model?.heading,
      bio: data?.model?.bio,
      location: {
        city: data?.model?.location?.city,
        country: data?.model?.location?.country,
      },
      stats: {
        age: data?.model?.stats?.age?.toString(),
        height: data?.model?.stats?.height?.toString(),
        heightType: data?.model?.stats?.heightType,
        weight: data?.model?.stats?.weight?.toString(),
        weightType: data?.model?.stats?.weightType,
        cockSize: data?.model?.stats?.cockSize?.toString(),
        cockSizeType: data?.model?.stats?.cockSizeType,
        foreskin: data?.model?.stats?.foreskin,
        sexualRole: data?.model?.stats?.sexualRole,
        tribe: data?.model?.stats?.tribe,
        bodyType: data?.model?.stats?.bodyType,
        orientation: data?.model?.stats?.orientation,
        eyeColor: data?.model?.stats?.eyeColor,
        bodyHair: data?.model?.stats?.bodyHair,
        smoking: data?.model?.stats?.smoking,
        drinking: data?.model?.stats?.drinking,
        piercings: data?.model?.stats?.piercings,
        tattoos: data?.model?.stats?.tattoos,
        condomsOnly: data?.model?.stats?.condomsOnly,
        hivStatus: data?.model?.stats?.hivStatus,
        hobbies: data?.model?.stats?.hobbies,
        spokenLanguages: data?.model?.stats?.spokenLanguages,
        hairColor: data?.model?.stats?.hairColor,
        amInto: data?.model?.stats?.amInto,
      },
    },
    validationFunction: (val) => isModelProfileValid(val, isPlan),
  ...
  useEffect(() => {
    if (data) {
      ...
      setCockSizeStatus(data?.model?.stats?.cockStatus);
        ...
        <div className="block w-full bg-white rounded-xl border border-secondaryShade3 px-4 md:px-6 py-4 md:py-6 custom-img-color">
          ...
          <div className="flex items-center justify-end w-full  pe-2">
            <span className="text-black ps-4 text-sm">
              {values?.bio?.length}/{isPlan == "Gent" ? "300" : "500"}
            </span>
          </div>
